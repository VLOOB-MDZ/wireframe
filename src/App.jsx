...
import { makeDefaults, PALETTES, PALETTE_NAMES, FRAME_PRESETS } from './vloob/constants.js';
import { renderModule } from './vloob/renderers.js';
import { Icon } from './components/icons.jsx';
import { Section, Slider, Toggle, Dropdown, Segmented, ColorField, TextInput } from './components/controls.jsx';
import { setIn, FramePanel, DrawingStudioPanel, GeoGeneratorPanel, HeroComposerPanel } from './components/panels.jsx';
import { Landing } from './components/Landing.jsx';
import { MobileStudio } from './components/MobileStudio.jsx';
...
const SETTINGS_KEY = 'wireframe-settings-v1';
...
function useSettings() {
  ...
  useEffect(()=>{ try { localStorage.setItem(SETTINGS_KEY, JSON.stringify(s)); } catch {} }, [s]);
  const set = (k, v)=> setS(p=> ({...p, [k]: v}));
  const reset = ()=> setS(SETTINGS_DEFAULTS);
  return [s, set, reset];
}
...
export default function App() {
  const [t, setTweak, resetTweaks] = useSettings();
  const [module, setModule] = useState('frame');
  ...
  const [autoSpeed, setAutoSpeed] = useState(2.2);
  const [paletteLock, setPaletteLock] = useState('random');
  const [noticeOn, setNoticeOn] = useState(true);
  useEffect(()=>{ document.body.setAttribute('data-notice-open', String(noticeOn)); }, [noticeOn]);
  ...
  useEffect(()=>{
    const img = new Image(); img.onload = ()=> { setImage(img); sampleRef.current = img; }; img.src = '/sample.jpeg';
  ...
  useEffect(()=>{ if(!autoOn) return; const id=setInterval(randomizeStyle, autoSpeed*1000); return ()=>clearInterval(id); }, [autoOn, autoSpeed, randomizeStyle]);

  // first impression: land on a generated look
  useEffect(()=>{ randomizeStyle(); }, []); // eslint-disable-line

  // ---- global history (undo / redo) ----
  const history = useRef({ stack: [], idx: -1 });
  const [, setHistVer] = useState(0);
  const skipHist = useRef(false);
  const curSrcRef = useRef(null);
  ...
  const clearGrade = ()=> setState(st=> ({...st, global:{...st.global, grade:{...st.global.grade, enabled:false}}}));
    ...
    <>
      ...
      {mobileMode ? (
        <MobileStudio
          ...
          frameShape={state.frame.shape} onFramePreset={applyFramePreset}
          grade={state.global.grade} onPalette={applyPalette} onClearGrade={clearGrade}
        ...
        <div className={'sidebar'+(drawerOpen?' open':'')}>
          ...
          <div className="stylelab">
            <Section title="✦ Style Lab">
              ...
              <Dropdown label="Palette" value={paletteLock} options={[{v:'random',label:'Random palette'}, ...PALETTE_NAMES.map((n,i)=>({v:String(i),label:n}))]} onChange={setPaletteLock} />
            </Section>
          </div>
          ...
          <Section title="Appearance" collapsible>
            <Dropdown label="Theme" value={t.theme} options={[{v:'carbon',label:'Carbon (dark)'},{v:'ink',label:'Ink (warm)'},{v:'slate',label:'Slate (cool)'},{v:'paper',label:'Paper (light)'}]} onChange={v=>setTweak('theme',v)} />
            <Dropdown label="UI font" value={t.uiFont} options={['JetBrains Mono','Space Grotesk','Oswald','Archivo']} onChange={v=>setTweak('uiFont',v)} />
            <ColorField label="Accent" value={t.accent} onChange={v=>setTweak('accent',v)} />
            <Segmented label="Density" value={t.density} options={['compact','regular','comfy']} onChange={v=>setTweak('density',v)} />
            <Dropdown label="Canvas backdrop" value={t.framing} options={[{v:'checker',label:'Checker'},{v:'solid',label:'Solid'},{v:'dots',label:'Dots'},{v:'grid',label:'Grid'}]} onChange={v=>setTweak('framing',v)} />
            <Segmented label="Topbar tabs" value={t.topbar} options={[{v:'center',label:'Center'},{v:'left',label:'Left'},{v:'segment',label:'Segment'}]} onChange={v=>setTweak('topbar',v)} />
            <button className="mini-btn" onClick={resetTweaks}>Reset Appearance</button>
          </Section>
        </div>
      </div>
      )}
...
function ExportModal({ onClose, onDownload, onCopy, canvasSize }) {
  ...
  const [filename, setFilename] = useState('wireframe-asset');
