...
import { CANVAS_FONTS, WEIGHTS } from './constants.js';
...
export function renderFrame(ctx, S, img, W, H, g) {
...
export function renderDrawing(ctx, S, img, W, H, g, live) {
  if (g.includeBackground) { ctx.fillStyle = g.backgroundColor; ctx.fillRect(0,0,W,H); }
...
export function renderGeo(ctx, S, img, W, H, g) {
  if (g.includeBackground) { ctx.fillStyle = g.backgroundColor; ctx.fillRect(0,0,W,H); }
  if (img) drawCover(ctx, img, 0,0,W,H); else placeholderBG(ctx,W,H);
  gradeImage(ctx, W, H, g.grade);
...
export function renderHero(ctx, S, img, W, H, g) {
...
// ---- dispatch ----
export function renderModule(canvas, module, state, img, live) {
