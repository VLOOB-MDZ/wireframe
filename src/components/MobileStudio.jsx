...
import { PALETTES, PALETTE_NAMES, FRAME_PRESETS } from '../vloob/constants.js';
...
export function MobileStudio({
  ...
  const curPalIdx = PALETTES.findIndex(p => grade.enabled && p[0]===grade.shadowColor && p[1]===grade.highlightColor);
      ...
      <div className="mobile-panel">
        ...
        <div className="quick-section">
          ...
          <div className="quick-row">
            ...
            {PALETTES.map((p,i)=>(
              <button key={i} className={'quick-chip-color'+(curPalIdx===i?' on':'')} onClick={()=>onPalette(i)} title={PALETTE_NAMES[i]} disabled={!hasImage}>
        ...
        <div className="quick-section">
          ...
          <div className="quick-row">
            {MOBILE_PRESET_ORDER.map(name => (
              <button key={name} className={'quick-shape'+(frameShape===FRAME_PRESETS[name].shape?' on':'')} onClick={()=>onFramePreset(name)} disabled={!hasImage} title={name}>
