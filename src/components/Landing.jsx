...
import { PALETTES, PALETTE_NAMES } from '../vloob/constants.js';
import { renderModule } from '../vloob/renderers.js';
import { Icon } from './icons.jsx';

export function Landing({ state, image, module, randomizeStyle, autoOn, setAutoOn, paletteLock, setPaletteLock, canvasSize, setCanvasSize, sizePresets, onEnter, onUpload }) {
  const cRef = useRef(null);
  useEffect(() => {
    const c = cRef.current; if (!c) return;
    if (c.width !== canvasSize.w || c.height !== canvasSize.h) { c.width = canvasSize.w; c.height = canvasSize.h; }
    renderModule(c, module, state, image, null);
    ...
    <div className="landing">
      ...
      <div className="lz-grid">
        <div className="lz-copy">
          ...
          <p className="lz-sub">Drop in an image and we&apos;ll map it into duotone, technical line-art compositions — circles, grids, hero layouts. Hit shuffle until it clicks, then fine-tune everything in the Studio.</p>
