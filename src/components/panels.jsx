...
import { FONT_LIST, WEIGHTS, FRAME_PRESETS, COLOR_PRESETS } from '../vloob/constants.js';
import { Icon } from './icons.jsx';
import { Section, Slider, Toggle, Dropdown, Segmented, ColorField, TextInput } from './controls.jsx';
...
export function FramePanel({ S, setS, hasImage }) {
...
export function DrawingStudioPanel({ S, setS, hasImage, onUndo, onRedo, onClear, canUndo, canRedo }) {
  const f = (p,v)=> setS(setIn(S,p,v));
  const veil = hasImage?'':'disabled-veil';
  return (
    <>
...
// ============ GEO GENERATOR PANEL ============
export const PATTERNS = [{v:'schematic',label:'Schematic (line-art)'},{v:'grid',label:'Grid'},{v:'dots',label:'Dot Matrix'},{v:'triangle',label:'Triangle Mesh'},{v:'rings',label:'Concentric Rings'},{v:'radial',label:'Radial Lines'},{v:'hexagon',label:'Hexagon Grid'},{v:'scatter',label:'Random Scatter'},{v:'diagonal',label:'Diagonal Stripe'}];
export const BLENDS = [{v:'normal',label:'Normal'},{v:'multiply',label:'Multiply'},{v:'screen',label:'Screen'},{v:'overlay',label:'Overlay'},{v:'difference',label:'Difference'},{v:'dodge',label:'Color Dodge'}];
...
export function HeroComposerPanel({ S, setS, hasImage }) {
